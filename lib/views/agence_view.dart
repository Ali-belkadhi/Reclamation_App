...
class _AgenceViewState extends State<AgenceView> {
  ...
  Future<void> _loadAgences() async {
    ...
    try {
      ...
      agences.sort((left, right) => left.nom.toLowerCase().compareTo(right.nom.toLowerCase()));
      if (!mounted) return;
      setState(() {
        _agences = agences;
        _isLoading = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Gestion des agences', style: TextStyle(fontWeight: FontWeight.w800)),
  ...
  Widget _buildContent() {
    ...
    if (agences.isEmpty) {
      return const Center(
        child: Text('Aucune agence trouvée', style: TextStyle(color: AppColors.textLight)),
      );
    }
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, setDialogState) {
          Future<void> save() async {
            ...
            try {
              agence == null ? await _service.create(value) : await _service.update(value);
              if (!dialogContext.mounted) return;
              Navigator.of(dialogContext).pop();
              await _loadAgences();
              if (mounted) {
                _showMessage(agence == null ? 'Agence ajoutée avec succès.' : 'Agence modifiée avec succès.');
              }
            } on AgenceException catch (error) {
              if (!dialogContext.mounted) return;
          ...
          return AlertDialog(
            title: Text(agence == null ? 'Nouvelle agence' : 'Modifier l’agence'),
            content: SizedBox(
              width: 520,
              child: Form(
                key: formKey,
                child: SingleChildScrollView(
                  child: Column(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      _field(controllers['code']!, 'Code *', maxLength: 20, isRequired: true),
                      const SizedBox(height: 12),
                      _field(controllers['nom']!, 'Nom *', maxLength: 100, isRequired: true),
                      const SizedBox(height: 12),
                      _field(controllers['adresse']!, 'Adresse', maxLength: 255, maxLines: 2),
                      const SizedBox(height: 12),
                      _field(controllers['ville']!, 'Ville', maxLength: 100),
                      const SizedBox(height: 12),
                      _field(controllers['telephone']!, 'Téléphone', maxLength: 20, keyboardType: TextInputType.phone),
                      const SizedBox(height: 12),
                      _field(controllers['email']!, 'Email', maxLength: 100, keyboardType: TextInputType.emailAddress, isEmail: true),
                      const SizedBox(height: 12),
                      _field(controllers['directeur']!, 'Directeur', maxLength: 100),
                    ],
                  ),
                ),
              ),
  ...
  Widget _field(
    ...
    return TextFormField(
      ...
      validator: (value) {
        ...
        if (isEmail && text.isNotEmpty && !RegExp(r'^[^@\s]+@[^@\s]+\.[^@\s]+$').hasMatch(text)) {
  ...
  Future<void> _confirmDelete(Agence agence) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('L’agence « ${agence.nom} » sera définitivement supprimée.'),
        actions: [
...
class _AgenceCard extends StatelessWidget {
  ...
  const _AgenceCard({required this.agence, required this.onEdit, required this.onDelete});
        ...
        child: Row(
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.account_balance_rounded, color: AppColors.primaryRed),
            ),
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(agence.nom, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
                  const SizedBox(height: 4),
                  Text(agence.code, style: const TextStyle(color: AppColors.primaryRed, fontWeight: FontWeight.bold)),
                  if (agence.ville != null) _detail(Icons.location_on_outlined, agence.ville!),
                  if (agence.adresse != null) _detail(Icons.map_outlined, agence.adresse!),
                  if (agence.directeur != null) _detail(Icons.person_outline_rounded, agence.directeur!),
                  if (agence.telephone != null) _detail(Icons.phone_outlined, agence.telephone!),
                  if (agence.email != null) _detail(Icons.email_outlined, agence.email!),
                ],
              ),
            ),
            PopupMenuButton<String>(
              ...
              itemBuilder: (context) => const [
                ...
                PopupMenuItem(value: 'delete', child: ListTile(leading: Icon(Icons.delete_outline, color: AppColors.error), title: Text('Supprimer'))),
              ],
            ),
          ],
        ),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(child: Text(value, style: const TextStyle(color: AppColors.textLight, fontSize: 12))),
        ],
      ),
    );
  }
